...
typedef struct {
    char tiles[MAX_H][MAX_W]; // Масив символів карти
    int h, w;                 // Реальні розміри
} Map;

typedef struct {
    int x, y;          // Координати
    int active;        // Чи живий
    int frozen_timer;  // Таймер заморозки
} Monster;
...
int main() {
    ...
    // ТЕМНО-СІРА ПІДЛОГА (Dark Grey): Білий текст (тьмяний) на Чорному фоні
                ...
                if (tile == C_WALL) { 
                    ...
                    if (tile == C_KEY) { 
                        ...
                        if (danger) {
                            ...
                            // A_DIM (тьмяний) + Білий текст + Чорний фон = Темно-сірий
